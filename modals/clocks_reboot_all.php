	...
	<div class="form-group">
	    <div class="col-sm-12">
			<div class="fr">
			    ...
			    <button type="submit" class="btn btn-danger" name="submit" id="submit"><i class="fa fa-trash" aria-hidden="true"></i>&nbsp;REBOOT ALL</button>
			</div>
		</div>
	</div>
</form>
...
<script type="text/javascript">
    ...
    jQuery("#forms").on('click', '#submit', function(e) {
      ...
      if (valid == true) {
      	//validated
      	e.preventDefault();
      	var redbull = "<?php echo $REDBULL; ?>";
		var confirm = jQuery("select#confirm").val();

        var datastr = 'reboot='+redbull+'&confirm='+confirm;
        //console.log(datastr);
        send(datastr);
      } else {
      	e.preventDefault();
    ...
    function send(data) {
        jQuery.ajax({
            type: "POST",
            url: "ajax/clocks_reboot_all.php",
            data: data,
            success: function(){
                jQuery("#forms").hide();
                jQuery("#success").show();
